...
exports.getById = async (req, res) => {
  try {
    ...
    if (member) {
      ...
      return res
        .status(400)
        .send({status: false, message: "ไม่พบข้อมูลผู้ใช้งาน"});
    }
  } catch (err) {
    console.log(err);
    return res.status(500).send({message: "มีบางอย่างผิดพลาด"});
...
exports.update = async (req, res) => {
  try {
    ...
    if (member) {
      return res
        .status(200)
        .send({status: true, message: "อัพเดตข้อมูลผู้ใช้งานสำเร็จ"});
    } else {
      return res
        .status(400)
        .send({status: false, message: "อัพเดตข้อมูลผู้ใช้งานไม่สำเร็จ"});
    }
  } catch (err) {
    console.log(err);
    return res.status(500).send({message: "มีบางอย่างผิดพลาด"});

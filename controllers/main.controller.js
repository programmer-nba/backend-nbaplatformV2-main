...
exports.login = async (req, res) => {
  try {
    ...
    if (member && (await bcrypt.compare(password, member.password))) {
      ...
      return res
        .status(400)
        .send({message: "เบอร์โทรหรือรหัสผ่านไม่ถูกต้อง กรุณาตรวจสอบอีกครั้ง"});
    }
  } catch (err) {
    console.log(err);
    return res.status(500).send({message: "มีบางอย่างผิดพลาด"});
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    const encrytedPassword = await bcrypt.hash(req.body.password, 10);
    const change_password = await Member.findByIdAndUpdate(decode._id, {
      password: encrytedPassword,
    });
    if (change_password) {
      return res
...
exports.genPublicToken = async (req, res) => {
  try {
    const token = jwt.sign(
      {code: "nbadigital", name: "NBA Digital Service", auth: "public"},
      process.env.TOKEN_KEY
    );
    if (token) {
      return res.status(200).send({status: true, token: token});
